...
export async function getYnabBudgets() {
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to fetch YNAB budgets",
    }
  }
}

export async function getYnabAccounts(budgetId: string) {
  try {
    const accessToken = await getYnabAccessToken()
    ...
    const accounts = response.data.data.accounts.filter(
      (account: YnabAccount) => account.on_budget && !account.deleted && !account.closed,
    )
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to fetch YNAB accounts",
    }
  }
}

export async function createYnabAccount(budgetId: string, name = "🤝 Splitwise") {
  try {
    const accessToken = await getYnabAccessToken()
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to create YNAB account",
    }
  }
}
