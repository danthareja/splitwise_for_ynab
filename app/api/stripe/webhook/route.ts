  ...
  switch (event.type) {
    ...
    case "customer.subscription.deleted": {
      ...
      if (primaryUser?.email) {
        ...
        // Email primary user (dedup key includes subscription ID so re-cancellations still send)
        const expiredEmailKey = `subscription.expired:${deletedSubscription.id}`;
        const expiredAlreadySent = await prisma.emailSend.findFirst({
          where: { userId: primaryUser.id, emailKey: expiredEmailKey },
        });
        ...
        if (primaryUser.secondaryUser?.email) {
          const secondaryExpiredEmailKey = `subscription.expired.secondary:${deletedSubscription.id}`;
          const secondaryExpiredAlreadySent = await prisma.emailSend.findFirst({
            where: {
              userId: primaryUser.secondaryUser.id,
              emailKey: secondaryExpiredEmailKey,
            },
          });
