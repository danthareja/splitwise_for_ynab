...
export default async function DashboardPage() {
  ...
  const syncHistory = syncHistoryResult.success ? (syncHistoryResult.syncHistory || []) : []

  // Check if user is fully configured
  const isFullyConfigured = hasSplitwiseConnected && hasYnabConnected && userSettings && ynabSettings
            ...
            <div className="text-center py-8">
              ...
              {isFullyConfigured ? (
                <p className="text-sm text-gray-400">
                  Flag a transaction in YNAB or click the sync button to get started
                </p>
              ) : (
                <p className="text-sm text-gray-400">Complete your configuration to start syncing</p>
              )}
            </div>
          )}
        </div>
